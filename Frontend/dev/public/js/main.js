...
async function main(){
    if(navigator.onLine){
        ...
        image.onload = function (){
            ...
            const grd = ctx.createLinearGradient(0, 800, 0, 400);
            console.log(imageData);

            grd.addColorStop(0, imageData.primaryColor);
...
async function loadImageData(){
    ...
    return fetch(url.toString())
        ...
        .then(body => {
            return {imageUrl: `http://localhost:3000/image/${body.image}`, primaryColor: body.hexcodes[0]};
        });
}
/*
